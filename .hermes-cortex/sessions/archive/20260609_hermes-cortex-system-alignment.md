...
- `auto-save-session` (koscap-works) — now writes to `project_current_session.md`
- `auto-save-session` (koscap-royalty) — created, replaces 5 one-shot session-update jobs
- All session-update-* one-shot jobs (koscap-royalty) removed
