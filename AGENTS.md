...
>
> **RULE 1: LOAD TASK-START FIRST — `skill_view('task-start')` is your first tool call on every task.** Nothing precedes it; a task not preceded by it is a trust violation. It bundles the pre-task sequence: cache_search → begin_change → always-skills (task-start, agent-flow, reflexion-check, change-checklist, survey-before-action, agent-contract, test-driven-development) → classify → work. **Exact tool names (double underscores):** `mcp__loop_governance__cache_search` → `mcp__loop_governance__begin_change` → `mcp__loop_governance__feedback_accept` → `mcp__loop_governance__end_change`. Load all 7 always-skills in ONE turn (loads accumulate across turns — never reload one you already have). A block message states its gate (GATE 1 of 2 / GATE 2 of 2) and the exact call to make: follow it, don't read the enforcer source. If `begin_change` is absent from your tool list the MCP server isn't registered here — check `hermes mcp list` (lock-free), report to your orchestrator, never hand-write a lock file. To see what YOUR host's enforcer actually does — gate timing, read-only matrix, block-message text, skill-credit state — run `python3 ~/.hermes-cortex/scripts/probe-gate-logic.py` (read-only, lock-free, exit 0 = matches docs); that is the diagnostic for "am I stuck on a real gate or on broken plumbing?"
>
> **RULE 2: USE LOOP GOVERNANCE ALWAYS.** Every change: `mcp__loop_governance__begin_change` → work → `mcp__loop_governance__cycle_query` → `feedback_accept/override` → `mcp__loop_governance__end_change`. Write tools block without a lock. **Close out (2026-08-08; tightened 2026-09-23):** `end_change()` refuses until the cycle is closed out — meaning `composite > 0` **or** an explicit `unscored_reason` recorded by `feedback_accept`/`feedback_override`. An unscored close must say why. Decisions are compared by **class** (`LOOP 🔄 — keep iterating` counts as LOOP). `begin_change()` refuses while unclosed cycles exist. Never stack PENDING. **Hook bypass blocked:** `git -c core.hooksPath=...` / `GIT_CONFIG_*=...` blocked — NOT the `--no-verify` hatch (3 tolerated, 4th+ mandated).
>
...
29. **Claim/verify protocol (task model v3)** — workers `claim` a pending slice before working it (`task-db.py claim <id>`); never improvise strategy on a slice with a plan — follow the plan, and `unclaim` with the tool gap named if blocked. `report <id>` with evidence moves the slice to review; ONLY the orchestrator `verify`s (approve → completed, reject → back to in_progress with the gap). Never mark a slice completed without verified evidence — a self-reported done is a claim, not a fact.
