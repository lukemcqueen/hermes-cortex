...
11. **Score every change** — every code/config/script edit logged to loop-governance DB.
  > **⚡ Pre-commit scoring hook** auto-creates a cycle on every commit. No bypass. The hook also runs the mandatory adversarial gate (A2/A4) on every staged script — `--no-verify` is a logged, audited bypass and must never be used to ship a hook-rejected change. **Bounded escape hatch (2026-08-05):** 3 consecutive `--no-verify` commits are tolerated; the 4th+ is MANDATED — its push is blocked and the enforcer refuses further `--no-verify` git commands until a fully verified commit resets the counter (10-min `agent-no-verify-audit` watchdog alerts). The nginx-threat-pipeline no longer uses `--no-verify` — a fresh marker + blocklist allowlist sanction its commits while scoring/adversarial/append-only guards still run.
