...
11. **Score every change** — every code/config/script edit logged to loop-governance DB.
  > **⚡ Pre-commit scoring hook** auto-creates a cycle on every commit. No bypass. The hook also runs the mandatory adversarial gate (A2/A4) on every staged script — `--no-verify` is a logged, audited bypass and must never be used to ship a hook-rejected change. **Bounded escape hatch (2026-08-05):** 3 consecutive `--no-verify` commits are tolerated (logged + alerted); the 4th+ is MANDATED — pre-push blocks its push, the enforcer refuses further `--no-verify` git commands, and the 10-min `agent-no-verify-audit` watchdog screams until a fully verified commit resets the counter. The nginx-threat-pipeline no longer uses `--no-verify` at all — a fresh marker + blocklist-allowlist stages make the pre-commit hook sanction its commits while scoring/adversarial/append-only guards still run.
