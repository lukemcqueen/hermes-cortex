...
22. **Persistent cross-session tasks** — Use `task-db.py` for durable task tracking (personal + local fleet rows; honest local-only fleet semantics). See `task-persistence` skill.
