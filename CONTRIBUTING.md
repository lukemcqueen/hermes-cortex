...
- **Source scripts** in `src/scripts/` — watchdogs, installers, utilities
- **Canonical skills** in `src/skills/` — organized by domain
...
| `hermes-agent-skill-authoring` | In-repo SKILL.md format, frontmatter requirements, quality checks | When writing/editing a skill in `src/skills/` |
...
```bash
# Search scripts
search_files(pattern="<topic>", path="~/hermes-cortex/src/scripts/", target="files")
...
|------|------|-----------------|
| Source script | `src/scripts/<kebab-name>.py` or `.sh` | `cortex-update.sh` MAP (see MAP format below) |
| Canonical skill | `src/skills/<category>/<name>/SKILL.md` | `docs/SKILLS-MANIFEST.md` |
...
```bash
# Format:
register "src/scripts/your-script.py" \
...
```bash
register "src/scripts/my-daemon.py" \
...
|-------------|---------------|---------------|
| **New canonical skill** | `src/skills/<category>/<name>/SKILL.md` | Every agent can load your workflow |
| **New script** | `src/scripts/<name>.py` + MAP in cortex-update.sh | Every agent gets the tool on `cortex-update` |
...
|-------------|---------------|
| `src/scripts/<name>` | `~/.hermes-cortex/scripts/<name>` |
| `src/agent-inbox/` files | `~/.hermes-cortex/agent-inbox/` |
| `src/dashboard/` files | `~/.hermes-cortex/dashboard/` |
| `src/skills/<category>/<name>/` | `~/.hermes/skills/<category>/<name>/` (via symlink) |
...
> **Note:** Skills from `src/skills/` are symlinked during `cortex-update.sh` to
