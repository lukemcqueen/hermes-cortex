...
try:
    ...
    for candidate in [
        ...
        Path(__file__).resolve().parent.parent / "scripts",  # repo ops/scripts/ or tools/scripts/
    ]:
...
def score_progress(previous_output: str, current_output: str) -> float:
    ...
    if not current_output.strip():
        return 0.0
    # Diff-aware progress (2026-08-02): embedding only the first 1500 chars of
    # prev vs current made deep-file changes invisible — a small commit at line
    # 800 of a 32KB file scored cosine sim 1.0 → progress 0.0 → no_progress →
    # STOP on legitimate incremental work. Instead, compare the CHANGED REGION:
    # the unified diff between prev and current. The diff is small for small
    # commits, so its embedding changes meaningfully even when the full files
    # are 99% identical in the first window.
    import difflib
    diff_text = "\n".join(difflib.unified_diff(
        previous_output.splitlines(), current_output.splitlines(), lineterm=""))
    if not diff_text.strip():
        # No textual difference at all — genuinely no progress
        return 0.0
    prev_emb = embed(previous_output[:1500])
    diff_emb = embed(diff_text[:1500])
    if prev_emb is None or diff_emb is None:
        # Ollama unavailable — assume progress (optimistic)
        return 5.0
    # Similarity between the diff and the previous code: how much of the change
    # is novel vs a rehash. Low sim = novel work = more progress.
    sim = cosine_similarity(prev_emb, diff_emb)
    # Normalize: diff-vs-prev sim of ~0.4-0.6 is typical for a real change;
    # sim ~1.0 means the diff is a near-copy of existing code (suspicious).
    progress = (1.0 - sim) / 0.5 * 10
    # Small commits deserve a floor — a 39-line fix IS progress even if the
    # diff is small. Without this, tiny diffs score ~0 and get flagged
    # no_progress despite being correct incremental work.
    progress = max(progress, 5.0)
    # Cache boost for known-good patterns
