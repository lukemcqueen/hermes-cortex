...
| `docs/design/mycortex-dream-todo-bridge.md` | **dream → todo bridge (implemented 2026-08-06)** — turns dream output into actionable `bus.todos` items: Option A (monthly knowledge-gap → "learn X" todos, cap 4) + Option B (insight triage all tiers, cap 2), dedup + caps + tenant-scoping enforced in `dream-todo-bridge.py`. Ships with the bus.todos schema fix + todo-db.py error-propagation fix (the proposed foundation never existed; implementation record + verified AC checklist in the doc) |
