...
| `docs/design/learning-ledger.md` | **fleet learning ledger (F-001, implemented 2026-08-08)** — `learnings` schema on bus Postgres (orchestrator-only), INSERT-only collectors via `learnings.capture()` SECURITY DEFINER, scrub hook (email/IPv4/home-path → placeholders), dedup UNIQUE(route, content_hash), status lifecycle pending→evaluated→applied→verified→retired, impact -3..+3. Write path: `learning-collect.py` (all agents) → lib `learning_capture()` → `POST /api/learnings` (existing bus auth). Lifecycle path: `learning-ledger.py` (orchestrator-only) → `learnings.set_status(id, status, impact)` (v002). 21-check schema battery + 14-check CLI battery. Consumers wired by F-002/F-006 |
...
| `docs/elicit/2026-08-08_self-healing-auto-learning-elicit.md` | **self-healing + auto-learning elicitation (2026-08-08)** — deep mode: 7 sub-domains (capture/evaluation/verification/self-healing/bus/operator/autonomy), 25F+5NF, RICE/MoSCoW (12 Must), 5 user stories. Surveyed 65 crons + 8 web research sources. Party handoff |
| `docs/elicit/2026-08-08_self-healing-auto-learning-party.md` | **self-healing + auto-learning party verdict (2026-08-08)** — 6-role HC-Party: Option A (central learnings ledger + daily eval-harness regression gate + learning→task promotion) wins 7.65 vs distributed 5.65 vs external 3.15. 20 findings (P-01..P-20), 3 slices, cost bracketed; findings promoted to tasks DB |
