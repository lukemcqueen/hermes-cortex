...
| `docs/runbooks/push-metrics-nginx-deploy.md` | **Push-metrics reverse proxy (port xx005) deploy (2026-09-23)** — `agent-push-metrics` failing 1000+ runs. Root cause: the xx005 block is opt-in via `HERMES_SERVICES` and was never deployed (`conf.d/` absent). Root runbook + the duplicate-port / silent-removal caveats |
