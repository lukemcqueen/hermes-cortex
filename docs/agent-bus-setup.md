...
```
                    ...
                    └──────────────┬───────────────┘
                                   │ proxy to localhost:8903
                    ┌──────────────▼───────────────┐
                    │    Agent Bus Server (:8903)   │
...
| Migration | Bus on port 8903 (replaces legacy inbox on 8904) | Bus replaced the old file-based inbox. Legacy fallback stopped. |
...
#    CORTEX_BUS_FALLBACK_URL=...  (optional fallback; legacy: CORTEX_INBOX_URL)
...
|-----|----------|---------|-------------|
| `CORTEX_BUS_URL` | ✅ Yes | — | Bus server URL (e.g. `https://domain:13004` or `http://127.0.0.1:8903`) |
...
| `CORTEX_BUS_FALLBACK_URL` | ❌ No | — | Optional fallback URL (legacy name: `CORTEX_INBOX_URL`) |
| `CORTEX_BUS_AUTH` | ✅ Yes | — | Basic auth for nginx `user:password` — used for external bus connections (legacy: `CORTEX_BASIC_AUTH`, `CORTEX_INBOX_AUTH`) |
| `CORTEX_INBOX_URL` | ❌ No | — | DEPRECATED — use `CORTEX_BUS_FALLBACK_URL` instead |
| `CORTEX_BASIC_AUTH` | ❌ No | — | DEPRECATED — use `CORTEX_BUS_AUTH` instead |
| `CORTEX_INBOX_AUTH` | ❌ No | — | DEPRECATED — use `CORTEX_BUS_AUTH` instead |
...
CORTEX_BUS_AUTH=user:pass   (nginx Basic auth — required for external connections; legacy: CORTEX_BASIC_AUTH)
AGENT_NAME=moses

# Legacy fallback — deprecated in favor of CORTEX_BUS_FALLBACK_URL / CORTEX_BUS_AUTH:
CORTEX_BUS_FALLBACK_URL=https://old-inbox.example.com     # legacy name: CORTEX_INBOX_URL
CORTEX_BUS_AUTH=user:pass   # legacy names: CORTEX_INBOX_AUTH, CORTEX_BASIC_AUTH
```
...
| `inbox-flag.py` cron | Polls file inbox for new messages (no_agent) |
