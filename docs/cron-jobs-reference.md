...
| secret-leak-watchdog | no_agent | 0 */4 * * * | Scan for leaked credentials |
| stale-ref-watchdog | no_agent | 0 5 * * * | Check for stale file references |
| agent-learning-collector | no_agent | 0 */6 * * * | Collect skills delta + lessons |
| no-verify-audit | LLM+prompt | every 60m | Check for --no-verify commits |
| skill-report-request | no_agent | 0 2 * * 1 | Request skill reports from agents |
| skill-report-process | no_agent | 0 3 * * * | Process collected skill reports |
| skill-evaluate | LLM+prompt | 0 9 * * 2 | Evaluate custom skills for upstreaming |
