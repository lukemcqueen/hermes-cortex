...
1. **Server agents** (`health_method: "http"`): Moses' `orch-fleet-watchdog` cron (`*/5 * * * *`) fetches each agent's vector via HTTP.
...
4. Verify:
   ...
   # → {"v":[...], "h":"hostname", "t":...}
   ```
5. Moses' `orch-fleet-watchdog` picks it up automatically once the `health_url` is set in `~/.hermes-cortex/state/agent-registry.json`.
...
> Moved from AGENTS.md by `agents-doc-audit.py --prune --apply`
>>>>>>> c0ceff2 (docs: prune 3 sections from AGENTS.md to docs/ (confirmation protocol, config, cron table))
