...
#
#  One-line install:
#    curl -fsSL https://raw.githubusercontent.com/lukemcqueen/hermes-cortex/main/install.sh | bash
#
#  Or from a local clone:
#    git clone --depth 1 https://github.com/lukemcqueen/hermes-cortex.git
...
if [[ -z "$SCRIPT_DIR" || ! -f "${SCRIPT_DIR}/src/scripts/os-config.sh" ]]; then
  if [[ ! -f "${SCRIPT_DIR:-/dev/null}/src/scripts/os-config.sh" ]]; then
    ...
    REPO_URL="https://github.com/lukemcqueen/hermes-cortex/archive/refs/heads/main.tar.gz"
...
M2BPY
  ...
  if cmp -s "$M2B_PATH" "$M2B_TMP"; then
    rm -f "$M2B_TMP"
    skip "memory-to-brain.py up to date"
  else
    mv "$M2B_TMP" "$M2B_PATH"
    chmod +x "$M2B_PATH"
    info "  Updated memory-to-brain.py (inline version changed)"
  fi
else
  M2B_TMP=$(mktemp)
  cat > "$M2B_TMP" <<'M2BPY2'
...
def build_current_md(memory_entries: list[str], user_entries: list[str]) -> str:
    ...
    lines = [
        ...
        "",
        "> **Purpose:** This file is auto-generated by memory-to-brain.py.",
...
M2BPY2
  ...
  chmod +x "$M2B_PATH"
  ok
  info "  Installed memory-to-brain.py"
fi
...
if [[ -d "$LANGFUSE_DIR" ]] && docker ps --format '{{.Names}}' 2>/dev/null | grep -q "langfuse"; then
  ...
  if [[ ! -f "$LANGFUSE_COMPOSE" ]]; then
    ...
    if [[ -f "${SCRIPT_DIR}/docker-compose.langfuse.yml" ]]; then
      ...
      curl -fsSL "https://raw.githubusercontent.com/lukemcqueen/hermes-cortex/main/deploy/docker-compose.langfuse.yml" -o "$LANGFUSE_COMPOSE"
    fi
  fi
...
if [[ -f "$DASHBOARD_DEST/server.py" ]]; then
  ...
  if [[ -d "$REPO_DASHBOARD" ]]; then
    ...
    curl -fsSL "https://raw.githubusercontent.com/lukemcqueen/hermes-cortex/main/src/dashboard/server.py" -o "$DASHBOARD_DEST/server.py"
    curl -fsSL "https://raw.githubusercontent.com/lukemcqueen/hermes-cortex/main/src/dashboard/com.hermes.cortex-dashboard.plist" -o "$DASHBOARD_PLIST"
    mkdir -p "$DASHBOARD_DEST/static"
    curl -fsSL "https://raw.githubusercontent.com/lukemcqueen/hermes-cortex/main/src/dashboard/static/index.html" -o "$DASHBOARD_DEST/static/index.html"
  fi
