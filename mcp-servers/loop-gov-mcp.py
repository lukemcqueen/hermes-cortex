...
def _dogfood_auto_deploy() -> None:
    ...
    Logs to the MCP server's stderr (visible in systemd/journal).
    """
    repo_path = GOVERNANCE_REPO_PATH
    deploy_path = GOVERNANCE_DEPLOY_PATH

    if not repo_path.exists():
        print("[DOGFOOD] Cannot auto-deploy — repo source missing", file=__import__('sys').stderr)
        return
    ...
    try:
        ...
        print(f"[DOGFOOD] unlock warning: {e}", file=__import__('sys').stderr)

    # Step 2: Copy repo to deployed
    try:
        deploy_path.parent.mkdir(parents=True, exist_ok=True)
        _dogfood_shutil.copy2(repo_path, deploy_path)
        print(f"[DOGFOOD] Copied enforcer: {repo_path} → {deploy_path}")
    except OSError as e:
        print(f"[DOGFOOD] Copy failed: {e}", file=__import__('sys').stderr)
        return

    # Step 3: Disable and re-enable the plugin to load new code
    for subcmd in ["disable", "enable"]:
        try:
            ...
            print(f"[DOGFOOD] Plugin {subcmd} warning: {e}", file=__import__('sys').stderr)
    ...
    try:
        ...
        print(f"[DOGFOOD] lock warning: {e}", file=__import__('sys').stderr)
