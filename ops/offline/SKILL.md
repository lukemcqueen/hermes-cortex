...
| `offline_code gen "<prompt>" --model qwen2.5:3b` | Code gen with an explicit model (env `CODING_MODEL` pins it) |
...
- **Generation:** top-3 matching snippets injected as context → `qwen2.5:3b` (or your `CODING_MODEL`)
...
| qwen2.5:3b | ~1.9 GB | Code gen — single fleet model (also the judge) |
