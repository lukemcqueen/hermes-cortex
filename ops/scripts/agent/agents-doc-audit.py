...
def check_git_freshness(file_path):
    ...
    if git_dir:
        try:
            ...
            print("WARNING: failed to parse config timestamp — using defaults", file=sys.stderr)
    ...
    if args.repo:
        ...
        if missing:
            if args.json:
                ...
                for s in missing:
                    print(f"   - {s}")
                print("   Add them before committing, or use git commit --no-verify as a last resort.")
            sys.exit(1)
