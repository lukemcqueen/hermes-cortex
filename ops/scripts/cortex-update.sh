...
register "ops/scripts/agent/local-agent-agents-doc-audit.py"  "${CORTEX_DEPLOY_HOME}/scripts/local-agent-agents-doc-audit.py"
...
copy_file() {
  ...
  if $DRY_RUN; then
    ...
    # ── Add source header ────────────────────────────────────
    # Prepend a header showing the repo source path so every agent
