#!/usr/bin/env python3
"""remediation-sensor.py — Companion script for agent-fixer (auto-remediation LLM cron).
...
def check_agent_bus():
    ...
    if sys.platform == "darwin":
        ...
        # Check systemd user service — the inbox API on :8903
        svc_out, _, svc_rc = run("systemctl --user is-active agent-inbox.service 2>/dev/null")
        svc_ok = (svc_out.strip() == "active")
    
    # Probe endpoint on :8903 (CORTEX_INBOX_URL)
