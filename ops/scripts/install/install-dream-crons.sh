...
5. WRITE-BACK (mandatory): create \`~/brain/<agent>/dreams/\` if missing, write_file \`~/brain/<agent>/dreams/YYYY-MM-monthly.md\`, then append \`YYYY-MM | <title> | <one-line summary>\` to \`~/brain/<agent>/dreams/INDEX.md\` (create if missing). Agent name from \`hostname\` or AGENT_NAME env.
...
5. WRITE-BACK (mandatory): determine your agent name via \`hostname\` or AGENT_NAME env (esther, moses, joseph, kustos, gisu, titus). Create \`~/brain/<agent>/dreams/\` if missing, then write_file \`~/brain/<agent>/dreams/YYYY-MM-DD.md\` containing the digest. Then append a one-line entry to \`~/brain/<agent>/dreams/INDEX.md\` (create if missing) in format: \`YYYY-MM-DD | <short title> | <one-line summary>\`.
6. Before writing, read \`~/brain/<agent>/dreams/INDEX.md\` if it exists and reference 1-2 prior dreams in your digest when natural (the brain remembering its own dreaming).
...
4. SCRIPTURE CONNECTION: if \`~/brain/<agent>/bible/\` exists (check with your agent name from \`hostname\` or AGENT_NAME), read \`~/brain/<agent>/bible/INDEX.md\` and the most recent book file; connect the week's scripture insight to the week's actual work themes. If no bible dir exists, skip this step gracefully (do NOT fail — this keeps the dream portable to all agents).
5. Write a warm ~200-250 word dream: the insight, the pattern, the thread that ties the week together.
6. WRITE-BACK (mandatory): create \`~/brain/<agent>/dreams/\` if missing, write_file \`~/brain/<agent>/dreams/YYYY-MM-DD-weekly.md\`, then append \`YYYY-MM-DD | <title> | <one-line summary>\` to \`~/brain/<agent>/dreams/INDEX.md\` (create if missing).
