...
4. SCRIPTURE CONNECTION: if \`~/brain/<profile>/bible/\` exists (profile name — NOT hostname: check HERMES_PROFILE env, then ~/.hermes/profiles/*/ dirs, fall back to hostname/AGENT_NAME only if no profiles exist), read \`~/brain/<profile>/bible/INDEX.md\` and the most recent book file; connect the week's scripture insight to the week's actual work themes. If no bible dir exists, skip this step gracefully (do NOT fail — this keeps the dream portable to all agents).
