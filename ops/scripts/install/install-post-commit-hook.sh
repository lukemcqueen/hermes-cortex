...
NOTIFY_SCRIPT="${REPO_DIR}/ops/scripts/manage/post-commit-notify.sh"
...
if [ -n "$REPO_DIR" ] && [ -f "${REPO_DIR}/ops/scripts/manage/post-commit-notify.sh" ]; then
  bash "${REPO_DIR}/ops/scripts/manage/post-commit-notify.sh" &
fi
HOOK

chmod +x "$HOOK_PATH"
echo "✓ Installed post-commit hook at ${HOOK_PATH}"
echo "  → Runs ops/scripts/manage/post-commit-notify.sh after every commit"