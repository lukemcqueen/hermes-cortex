#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
# setup-cortex-bus.sh — Full Bus pipeline setup
#
# Run ONCE on any machine to create the complete Agent Bus
# monitoring pipeline:
#  1. bus-watch (no_agent, every 10m) — detects messages
#  2. cortex-bus (LLM-driven, every 30m) — reads + acts
#
# Usage:
#  bash setup-cortex-bus.sh       # auto-detect agent
#  bash setup-cortex-bus.sh --agent titus
#  bash setup-cortex-bus.sh --dry-run
#
# Requires:
#  - Hermes CLI (hermes cron create)
#  - ~/.hermes-cortex/cortex-bus.conf with CORTEX_INBOX_AUTH
#  - curl (for API calls)
#  - Pulled hermes-cortex (for bus-watch.sh)
#
# After setup:
#  hermes cron list | grep -E 'bus-watch|cortex-bus'
# ─────────────────────────────────────────────────────────────
set -euo pipefail

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'
BOLD='\033[1m'; CYAN='\033[0;36m'; RESET='\033[0m'
info() { echo -e "${GREEN}✓${RESET} $*"; }
warn() { echo -e "${YELLOW}⚠${RESET} $*"; }

DRY_RUN=false
AGENT=""

for arg in "$@"; do
 case "$arg" in
  --dry-run) DRY_RUN=true ;;
  --agent=*) AGENT="${arg#--agent=}" ;;
  --agent) shift; AGENT="$1" ;;
 esac
done

# ── Resolve agent identity ──────────────────────────────
if [ -z "$AGENT" ]; then
 AGENT="${AGENT_NAME:-${HERMES_AGENT:-}}"
fi
if [ -z "$AGENT" ] && [ -f "${HOME}/.hermes/agent-name" ]; then
 AGENT=$(tr -d '[:space:]' < "${HOME}/.hermes/agent-name")
fi
if [ -z "$AGENT" ]; then
 HOST=$(hostname -s 2>/dev/null || echo "unknown")
 case "$HOST" in
  orchestrator-1|moses*)  AGENT="moses" ;;
  worker-1|gisu*)     AGENT="gisu" ;;
  worker-2|joseph*)    AGENT="joseph" ;;
  worker-3|kustos*)    AGENT="kustos" ;;
  worker-5|esther*)    AGENT="esther" ;;
  LAM2|titus*)       AGENT="titus" ;;
  *) echo "Cannot detect agent. Use --agent=<name> or set AGENT_NAME."; exit 1 ;;
 esac
fi

INBOX_URL="${CORTEX_BUS_URL:-${CORTEX_BUS_FALLBACK_URL:-${CORTEX_INBOX_URL:-https://your-domain.com:13004}}}"

echo ""
echo -e "${BOLD}━━━ Agent Bus Pipeline Setup ━━━${RESET}"
echo -e " Agent: ${CYAN}${AGENT}${RESET}"
echo ""

# ── Prerequisites ───────────────────────────────────────────
if ! command -v curl &>/dev/null; then
 warn "curl not found — install it first"; exit 1
fi

# Check Hermes CLI
HERMES_CMD=""
for cmd in "${HOME}/.bun/bin/hermes" "${HOME}/.local/bin/hermes" "$(command -v hermes 2>/dev/null || true)"; do
 [ -x "$cmd" ] && HERMES_CMD="$cmd" && break
done

if [ -z "$HERMES_CMD" ]; then
 warn "Hermes CLI not found. Cannot create crons automatically."
 warn "Install Hermes first, then re-run this script."
 exit 1
fi

# Check bus-watch.sh exists
WATCH_SCRIPT="${HOME}/.hermes-cortex/scripts/bus-watch.sh"
if [ ! -f "$WATCH_SCRIPT" ]; then
 warn "bus-watch.sh not found — running cortex-update.sh..."
 if [ -f "${HOME}/hermes-cortex/ops/scripts/cortex-update.sh" ]; then
  bash "${HOME}/hermes-cortex/ops/scripts/cortex-update.sh" 2>/dev/null || true
 fi
 if [ ! -f "$WATCH_SCRIPT" ]; then
  warn "Still not found. Run: cd ~/hermes-cortex && git pull && bash ops/scripts/cortex-update.sh "
  exit 1
 fi
fi

# Check auth config
AUTH_FILE="${HOME}/.hermes-cortex/cortex-bus.conf"
AUTH_OK=false
if [ -f "$AUTH_FILE" ]; then
 # shellcheck disable=SC1090
 source "$AUTH_FILE" 2>/dev/null || true
 TEST_AUTH="${CORTEX_INBOX_AUTH:-}"
 if [ -n "$TEST_AUTH" ]; then
  AUTH_OK=true
 fi
fi
if ! $AUTH_OK; then
 warn "No auth credentials found in ~/.hermes-cortex/cortex-bus.conf"
 warn "Create it with:"
 echo "  CORTEX_INBOX_URL=\"https://your-domain.com:13004\""
 echo "  CORTEX_INBOX_AUTH=\"user:pass\""
 echo "  AGENT_NAME=\"${AGENT}\""
fi

# ── Test connectivity ───────────────────────────────────────
echo -n " Testing inbox API connectivity... "
if $AUTH_OK; then
 API_OK=$(curl -sf --max-time 10 \
  -u "$CORTEX_BASIC_AUTH" \
  "${INBOX_URL}/health" \
  >/dev/null 2>&1 && echo "true" || echo "false")
else
 API_OK=$(curl -sf --max-time 10 "${INBOX_URL}/health" >/dev/null 2>&1 && echo "true" || echo "false")
fi

if [ "$API_OK" = "true" ]; then
 info "reachable"
else
 warn "cannot reach ${INBOX_URL}/health"
 warn "Check connectivity and credentials."
fi

# ── Save agent identity ─────────────────────────────────────
if [ ! -f "${HOME}/.hermes/agent-name" ]; then
 mkdir -p "${HOME}/.hermes"
 echo "$AGENT" > "${HOME}/.hermes/agent-name"
 info "Saved agent identity to ~/.hermes/agent-name"
fi

if $DRY_RUN; then
 echo ""
 echo -e "${YELLOW}[DRY RUN] Would create:${RESET}"
 echo " 1. bus-watch (no_agent, every 10m, deliver=local)"
 echo " 2. cortex-bus (LLM, every 30m, context_from=bus-watch)"
 echo ""
 exit 0
fi

# ── Step 1: Create bus-watch cron ─────────────────────────
echo ""
echo -e " ${BOLD}Step 1/2: Creating bus-watch detector...${RESET}"
EXISTING=$("$HERMES_CMD" cron list 2>/dev/null | grep -c "bus-watch" || true)
if [ "$EXISTING" -gt 0 ]; then
 warn "bus-watch cron already exists — skipping"
else
 "$HERMES_CMD" cron create \
  name=bus-watch \
  schedule="*/10 * * * *" \
  script=bus-watch.sh \
  no_agent=true \
  deliver=local 2>&1 | sed 's/^/  /'
 info "bus-watch cron created (every 10 minutes)"
fi

# ── Get bus-watch job ID ──────────────────────────────────
WATCH_JOB_ID=$("$HERMES_CMD" cron list 2>/dev/null | grep "bus-watch" | head -1 | awk '{print $1}')
if [ -z "$WATCH_JOB_ID" ]; then
 warn "Could not find bus-watch job ID — will need manual fix"
 WATCH_JOB_ID="REPLACE_ME"
fi

# ── Step 2: Create cortex-bus processor cron ───────────────
echo ""
echo -e " ${BOLD}Step 2/2: Creating cortex-bus processor...${RESET}"
EXISTING=$("$HERMES_CMD" cron list 2>/dev/null | grep -c "cortex-bus" || true)
if [ "$EXISTING" -gt 0 ]; then
 warn "cortex-bus cron already exists — skipping"
else
 # Build prompt with agent-specific values substituted
 PROMPT="Process messages from the Agent Bus for ${AGENT} using the Inbox Message Decision Framework.
Read the context from bus-watch (lists unread message subjects and senders).

For each unread message:

1. Fetch the full message via the inbox API:
  \`\`\`bash
  source ~/.hermes-cortex/cortex-bus.conf
  curl -s -u \"\$CORTEX_BASIC_AUTH\" \\
   \"${INBOX_URL}/api/pgmq/read\" -d '{\"queue\":\"inbox_${AGENT}\",\"vt\":30}'
  \`\`\`

2. Apply the Decision Framework:
  - Priority: critical→act now, urgent→this tick, normal→this tick
  - Can you fix it yourself? → AUTO-ACT using terminal/web/file tools. Fix it, verify it, report it.
  - Needs another agent? → Use hermes cron/inbox_send to delegate, CC the user
  - Needs human judgment? → Output a clear escalation summary with options
  - FYI only? → Acknowledge and move on

3. Report what you did in a clear summary.

IMPORTANT: Use https://your-domain.com:13004 for API calls (not localhost).
The inbox MCP tool does not work from this machine."

 "$HERMES_CMD" cron create \
  name=cortex-bus \
  schedule="*/30 * * * *" \
  context_from="${WATCH_JOB_ID}" \
  enabled_toolsets="web,terminal,file" \
  prompt="${PROMPT}" \
  deliver=origin 2>&1 | sed 's/^/  /'
 info "cortex-bus cron created (every 30 minutes, reads context from bus-watch)"
fi

# ── Test ────────────────────────────────────────────────────
echo ""
echo -n " Testing inbox-watch... "
TEST_OUTPUT=$(AGENT_NAME="$AGENT" bash "$WATCH_SCRIPT" 2>/dev/null || true)
if [ -n "$TEST_OUTPUT" ]; then
 echo "found messages"
 echo ""
 echo "$TEST_OUTPUT" | head -12
 echo " (...)"
else
 info "no new messages (check back later)"
fi

# ── Summary ─────────────────────────────────────────────────
echo ""
echo -e "${GREEN}━━━ Pipeline setup complete for ${AGENT} ━━━${RESET}"
echo " Detector: bus-watch (every 10m, silent unless new)"
echo " Processor: cortex-bus (every 30m, reads + acts)"
echo ""
echo " Verify with:"
echo "  hermes cron list | grep -E 'bus-watch|cortex-bus'"
echo ""
