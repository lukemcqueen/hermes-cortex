...
def check_deploy_checksums(res):
  ...
  def _content_md5(path):
    ...
    cortex-update.sh prepends a 3-line SOURCE header to deployed .sh/.py
    files of the form:
      # SOURCE: <path>
      # Do NOT edit this file directly.

    This function strips those first 3 lines from the content before
    computing the hash, so checksums match the repo source. Returns
    None on error.
    """
    try:
      if not path.is_file():
      ...
      # Check if the file starts with the SOURCE header pattern
