    ...
    for conf in [REPO_DIR / ".env",
        ...
        if conf.exists():
            try:
                ...
                print("WARNING: config parse error in bus URL detection — using defaults", file=sys.stderr)
        if CORTEX_BUS_TOKEN:
            break
...
def git_commit_and_push(files: list[str], agent: str) -> str:
    ...
    try:
        ...
        # Create temp governance lock for automated commit
        gov_lock = os.path.expanduser("~/.hermes-cortex/state/.governance-hermes-cortex.json")
        os.makedirs(os.path.dirname(gov_lock), exist_ok=True)
        import json
        with open(gov_lock, "w") as f:
            json.dump({"task_id": "skill-triage-upstream", "repo_slug": "hermes-cortex", "session_id": "automated-skill-triage"}, f)

        # Commit (under governance lock)
