#!/usr/bin/env python3
"""
learnings migrate.py — schema_version-gated migration runner for the
learnings ledger schema (F-001).

Mirror of ops/services/tasks/migrate.py — same version-gated pattern, same
DDL-as-owner trust model. Applies SQL migrations from the sibling schema/
directory to the shared mycortex-postgres database (the same instance that
hosts the bus.* schema — the "bus Postgres" of the F-001 party doc).

Invoked by cortex-update.sh AFTER file sync (cortex-update itself has no DDL
path — this runner is the DDL path, same as ops/services/tasks/migrate.py).
Registered via register_orch in cortex-update.sh: only orchestrator hosts
(Moses/Esther) run the bus Postgres, so only they apply the learnings schema.
Workers never have the bus schema (docs/bus-architecture.md role matrix) and
write to the ledger via the HTTP collector path instead — they never need
this schema locally.
...
Migration discovery:
    - schema/vNNN__*.sql       → version NNN (v001__learnings.sql = version 1)

Design: docs/design/learning-ledger.md §schema (party L-1..L-6).
"""
...
# collector scripts connects as mycortex_reader_<profile> or goes through
# the bus HTTP API (server-side capture()) — never superuser.
DDL_ROLE = "mycortex"
SCHEMA = "learnings"
...
def _psql_base(db_name: str, role: str = DDL_ROLE) -> list[str]:
    ...
    # Linux — docker exec as DDL role. SQL flows via stdin (never embedded
...
def current_version(db_name: str) -> int:
    ...
    try:
        out = psql_query(
            f"SELECT COALESCE(MAX(version), 0) FROM {SCHEMA}.schema_version;", db_name
        )
...
def main() -> int:
    ...
    if not pending:
        print(f"{SCHEMA} schema up to date (version {current}) — no-op")
        return 0

    print(f"{SCHEMA} schema: current={current}, applying {len(pending)} migration(s) to DB '{args.db_name}'")
    applied_max = current
    for version, path in pending:
        ...
        ver_rc, ver_out, ver_err = psql_script(
            f"INSERT INTO {SCHEMA}.schema_version (version) VALUES ({version}) "
    ...
    print(f"{SCHEMA} schema now at version {applied_max}")
    return 0
