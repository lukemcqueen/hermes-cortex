...
def _psql_base(db_name: str) -> list[str]:
    ...
    # Linux — docker exec: prefer plain docker when the user can already
    # talk to the daemon (docker group, rootless, or Arch/Omarchy where the
    # `sg` binary ships in no default package). Fall back to `sg docker`
    # only when the group exists but this user isn't in it (Debian-family).
    import shutil
