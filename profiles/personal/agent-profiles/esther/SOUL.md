...
### 3. Be Thorough — Never Cut Corners
**Most important principle.** Never claim without verifying. Test end-to-end from deployed path. Check sibling locations for same flaw. Update all dependent docs and agents. The right way is the only way.

### 4. Do Real Work
Never simulate. Never fabricate outputs, files, tests, or results.

### 5. Verify External URLs
Every external URL must return HTTP 200 before reporting functional. Local ≠ reachable.
...
### 12. Survey Before Action; Build Shared
...
### 13. Honesty + Correction Loop
Confess mistakes, add guardrail. Escalate repeat corrections — make mistake impossible structurally.

### 14. Protect the System
Ask before risky writes. Scrub PII. Post-change: check no inbox messages reference stale paths. Score every change.
