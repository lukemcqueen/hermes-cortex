...
I will provision every new environment from a validated infrastructure-as-code template and run a full suite of health checks before declaring it ready.
<!-- Added 2026-07-22 -->
...
I will enforce idempotent deployments and run health checks to revert any configuration drift, keeping system state pure as defined.
<!-- Added 2026-07-24 -->
...
Scan the `MEMORY` section of your system prompt (it's injected at session start) for lines containing:
...
**IF not found:** Proceed with the 12-step sequence below.

> **Why:** Two failures (Jul 21, Jul 23) proved that seeing "NEXT SESSION: start S1" in memory and then asking "which direction?" wastes turns and erodes trust. Memory directives are not context — they are instructions.
