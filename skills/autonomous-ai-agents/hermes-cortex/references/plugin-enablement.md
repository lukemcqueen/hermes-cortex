...
The `install.sh` script creates the gbrain Hermes plugin files at `~/.hermes/plugins/gbrain-command/` but does NOT enable the plugin. Users will find the plugin shows as "not enabled" when running `hermes plugins list`, and the `/brain` slash command will not work.
...
2. ✅ Verify gbrain: `gbrain --version`
3. ✅ Verify launchd services: `launchctl list | grep -E "(ollama|gbrain)"`
4. ✅ Enable plugin: `hermes plugins enable gbrain-command`
5. ✅ Verify plugin: `hermes plugins list | grep gbrain`
6. ✅ Test in new session: Start new Hermes session and run `/brain test query`

## Related Files

- `~/.hermes/plugins/gbrain-command/__init__.py` — Plugin implementation
- `~/.hermes/plugins/gbrain-command/plugin.yaml` — Plugin configuration
- `~/.hermes/config.yaml` — Hermes config (plugin enablement tracked here)

## Session Evidence

From session 2026-06-04:
- Plugin files existed at `~/.hermes/plugins/gbrain-command/`
- `hermes plugins list` showed status as "not enabled"
- Ran `hermes plugins enable gbrain-command` → "✓ Plugin gbrain-command enabled. Takes effect on next session."
