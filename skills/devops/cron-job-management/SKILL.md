...
- **opencode-free provider is DEAD upstream — do not pin to it (2026-09-02).**
...
- **no_agent scripts that import from project modules need PYTHONPATH.** Cron jobs run in a bare environment — no PYTHONPATH, no project venv. If a no_agent Python script imports from a project module (e.g. `cortex_bus` under `~/hermes-cortex/core/cortex_bus/`), it must handle both at the script level:
