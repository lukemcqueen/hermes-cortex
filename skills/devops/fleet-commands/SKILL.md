...
**Critical rule: Moses does NOT run `agent-message-handler.py` on itself.** The handler is for fleet agents to process commands from the orchestrator. Moses handles its own inbox in-session (via tools) and out-of-session (via `agent-bus-*` LLM crons).
...
- **Handler runs on fleet agents only** — `agent-message-handler.py` processes up to **25 messages per 5-min tick** on fleet agents (esther, joseph, gisu, kustos). It does NOT run on the orchestrator (Moses). Previously (pre-2026-07-23, commit `9946158`) it processed only 1/tick, causing a 5+N-minute backlog for structured commands queued behind Learning Reports or health pings. Now the handler drains the queue on every tick, so backlog is rare. If commands still appear delayed, the agent has >25 backlogged messages or the handler fell behind.
...
- **push_only agents never poll** — Titus has `bus_access: client`. Sending a command to his inbox queues it successfully, but he has `health_method: inbox` and does not poll for bus commands. Use a forwarder or alternative channel for push-only agents.\n- **Active consumers archive immediately** — agents with `bus_access: host` that have a message handler will read and archive the message. You may not see it in `pending` if you check late.
