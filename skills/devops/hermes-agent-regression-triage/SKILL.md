...
0. **Many crons failing with the SAME exception at once = scheduler-level fault, not per-job — and the cause is often an UNCOMMITTED local edit, not upstream.** If a batch of unrelated agent crons (ai-brief, sustainability, job-scanner, backlog-driver) all report the identical `'_FireAudit' object has no attribute 'agent'`-style error within one window, stop auditing commits and check the SHARED firing path first: `git status --short` + `git diff cron/scheduler.py` on the hermes-agent tree. A stale uncommitted scratch edit (a prior session's half-applied change) re-introduces a broken line the committed base never had — `git blame` shows `Not Committed Yet` with today's timestamp. Repair the file directly (`python3 -m py_compile` + a minimal instantiate-and-call probe of the failing object), not by checking out a pinned tag. **FIRST check whether a cortex patcher re-injects that line.** Patchers in `~/hermes-cortex/ops/scripts/install/` (e.g. `install-cron-cost-tracking.py`) run on EVERY `cortex-update.sh`, so a hand-repair is silently undone at the next sync — the durable fix is the patcher's snippet in the repo, deployed, with a repair step that heals an already-broken host. **Hosts diverge on the injected text** (one sets `self.agent = agent`, another `self._agent = agent`); a snippet written against one AttributeErrors on the other, so normalise the name before injecting — otherwise the fix for one host is the breakage for the next to update. **Prefer config/extension points to patching core at all:** make a patch removable from CONFIG (e.g. `cron.cost_tracking.enabled: false`) so a broken optional feature is switched off without editing Python, and remember an audit-write bug fails EVERY cron because the write sits inside the job's `try`. A fix to a Hermes core file ships on that host only — it is not in the cortex repo.
