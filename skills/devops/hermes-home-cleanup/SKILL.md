...
| `~/.hermes-cortex/a2a/` | **DEPRECATED dir** — A2A service merged into agent-bus (migration docs `src/a2a/` → `ops/services/a2a/` → agent-bus). Bus server serves a HARDCODED card at `/.well-known/agent-card.json`; deployed card is `bus/agent-card.json` (register_orch in cortex-update.sh). Nothing reads `a2a/` — but `agent-card.json` there was regenerated 2026-07-27 by a stale `generate-agent-card.py --output` run, so verify nothing new writes it before deleting |
