...
| **Dying SSD — all errors are WRITE failures** | 100s of `I/O error, dev sda, sector N op 0x1:(WRITE)` spread across the whole disk, new sectors daily; postgres fdatasync PANICs | Signature of failing NAND, not a single defect (e.g. Apple SSD SM0128 in 2014 MBPs — known premature-failure model). Verify with `sudo smartctl -a /dev/sda` (Reallocated/Pending sector counts). Stopgap: back up NOW, then `e2fsck -fcc /dev/<root>` from recovery mode or live USB to quarantine bad blocks in the ext4 bad-block inode (a read-only `fsck -f` will MISS write-failing blocks — you need the read-write `-cc` scan). Real fix: replace the SSD; restore from backup. |
