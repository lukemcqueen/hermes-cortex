...
- **OTel "Failed to detach context" ERROR spam (cosmetic).** The hermes-agent Langfuse plugin manages OTel span contexts across async tasks; when a turn ends in a different asyncio context than it started, `opentelemetry.context` logs `ValueError: <Token var=<ContextVar name='current_context'...>> was created in a different Context` + `ERROR opentelemetry.context: Failed to detach context` at ERROR level (hundreds/day in `~/.hermes/logs/errors.log` and syslog). The plugin catches the exception (fail-open — traces still land correctly), so this is log noise that can mask real errors, not a data-loss bug. Started 2026-08-26 on esther (survives SDK 4.14.3 → container recreates). Proper fix is an upstream hermes-agent plugin change (suppress the `opentelemetry.context` logger or scope span end to the creating context); do not silence the whole root logger. Hermes config has no per-logger level override, so interim = tolerate the noise or pin/upgrade the langfuse SDK and re-check.
