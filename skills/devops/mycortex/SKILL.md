...
- ✅ **S-010 parity gate (2026-08-02):** wired to BOTH the doctor (`check_mycortex_parity` in cortex_doctor/checks.py) AND a daily `agent-mycortex-parity` no_agent cron (silent on pass, alerts on regression). Note: wired to doctor+cron, not pre-commit — the doctor/cron path is the enforcement.
