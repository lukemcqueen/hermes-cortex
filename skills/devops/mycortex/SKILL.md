...
- **Never test against the prod `mycortex` DB** — always `--db-name mycortex_test`.
...
- Doctor `mycortex daemon` check kept as decommission verification (PASS when the
...
- ✅ **S-010 parity gate (2026-08-02) — RETIRED 2026-08-03:** gate achieved its purpose (proved mycortex ≈ mycortex during migration; legacy brain now deprecated). Doctor check `check_mycortex_parity` reduced to INFO (no subprocess) and daily `agent-mycortex-parity` cron removed. Parity script kept as a **manual regression fixture** only (`mycortex-parity.py --mode check`).
...
- ⏳ Remaining: mycortex decommission phases (S-011 done: autopilot disabled, mycortex crons removed; S-012 tombstone/purge is time-gated — 30-day window after flip, do NOT force; S-013/S-014/S-015 = post-flip verify, v1.1 semantic, v1.2 MCP)
...
- **`mycortex down` watchdog alerts usually mean a STALE sync, not a down service.** `doctor --json` returns `ok:false` when any source's `last_sync_at` is >2h old (or NULL) — the health vector then reports -1 and a recurring fleet-watchdog alert fires. Diagnose in order: `mycortex doctor --json` (confirm stale-source warning) → `mycortex sources list` (check LAST_SYNC ages) → check the sync cron's state in `~/.hermes/cron/jobs.json` (`enabled:false` / `paused_at` set = the cause). If a one-shot `mycortex sync` and the script both run clean manually, resume the paused cron (`cronjob action=resume`) — do not debug a sync that isn't broken.
