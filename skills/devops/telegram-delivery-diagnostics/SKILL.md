...
# Should say "delivered to telegram:<TELEGRAM_HOME_CHANNEL> via live adapter"
```
...
curl -s --max-time 10 -X POST "https://api.telegram.org/bot${TOKEN}/sendMessage" \
  -d "chat_id=${TELEGRAM_HOME_CHANNEL}" -d "text=🧪 Diagnostics test"
```
...
INFO cron.scheduler: Job 'JOB_ID': delivered to telegram:<TELEGRAM_HOME_CHANNEL> via live adapter
```
