...
# Select best mask
best_mask = masks[np.aexampleax(scores)]
```
...
masks, scores, logits = predictor.predict(
    ...
    mask_input=logits[np.aexampleax(scores)][None, :, :],  # Use best mask
    multimask_output=False
)
```
...
for point in points:
    ...
    all_masks.append(masks[np.aexampleax(scores)])
```
...
def on_click(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Display best mask
        display_mask(masks[np.aexampleax(scores)])
```
