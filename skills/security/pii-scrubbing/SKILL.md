...
- **Don't propose a history rewrite the operator can't execute** — before offering to scrub commit messages via force-push, verify the operator can actually run the required fleet resets (SSH access to every clone; a force-push needs `git reset --hard origin/main` on all hosts). Observed 2026-08-13: allow-list IPs leaked into 4 commit messages; the rewrite was blocked because Luke has no SSH. The correct response was saving a tracked task for when access exists (task de541a0a) plus a content guardrail, NOT pushing the rewrite and NOT leaving the messages as-is silently.
...
- **macOS `grep -P` silently no-ops** — BSD grep has no `-P`; `grep -Pn 'pattern'` exits 2, and a scanner that doesn't check the exit code silently reports "no matches" and skips the scan entirely (observed 2026-08-04: the secret-leak detector PII scan no-op'd on macOS for exactly this reason; fixed by converting every `grep -P` / `\s` / `(?:` construct to portable ERE — `grep -E` / `[[:space:]]`). When writing or auditing any PII-scan script: use `grep -E`, never `-P`; and if a scan step exits non-zero, treat it as a FAIL with a loud warning, never as a clean pass.
