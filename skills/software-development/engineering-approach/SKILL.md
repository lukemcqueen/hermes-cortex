...
| `frontend-auth-redirect-pattern.md` | Session-expire redirect to `/login?redirect=<path>`, post-login redirect back to referring page. AuthProvider, AuthGuard, fetchJSON 401 handling, login page patterns. Test mock implications for `usePathname`/`useSearchParams`.
...
- **Python test env pitfalls**: `references/python-test-env-pitfalls.md` — `.env` variables sourced by `./run` leak into module-level `os.getenv()` constants before tests run; patching one constant misses others in the same code path.
...
- **Works/Royalty bounded contexts**: `references/works-royalty-bounded-contexts.md` — works/shares/contracts remain in acme-alpha (Rails) as source of truth; acme-royalty holds read-only projections for matching/distribution; sync layer boundary
...
- **ACME Works domain conventions**: `references/acme-works-domain-conventions.md` — acme-works domain vocabulary and entity conventions (works, creators, societies, rights). Use when touching acme-works models or API schemas.
...
- **Next.js locale routing**: `references/nextjs-locale-routing-pattern.md` — middleware, `[locale]` route group, locale-aware router wrapper, LocaleLink component, page migration steps, test adaptation. Plus Next.js 15 + React 19 build/route gotchas: empty `app/` dir shadowing `src/app/`, `useSearchParams()` Suspense boundary, React 19 `useRef()` API change, zod v4 + `@hookform/resolvers` compat
