...
│       │   ├── Config template? → Update nginx/*.conf or docs/
...
| Private | Public |
|---------|--------|
| `realgospelmessage.org` | `$DOMAIN` or `your-domain.com` |
...
| `nginx/hermes-services.conf` | Reverse proxy templates | After nginx config evolution |
