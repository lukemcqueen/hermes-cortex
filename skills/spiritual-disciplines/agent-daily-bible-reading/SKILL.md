...
- **Reasoning models burn the token budget on reasoning_content (45f51473, 2026-08-10)** → `deepseek-v4-flash` is a REASONING model: on the long brain-page prompt it consumed the entire max_tokens budget as `reasoning_content` and returned `content=''` with `finish=length` at every limit (2048-8192). The script treated that as "empty response" and retried — unbounded recursion until the curl timeout, then silently dropped the brain page while SOUL.md kept the entry (inconsistent state). **Fix:** use a NON-reasoning model (`deepseek-chat`) for this script's API calls — verified: full 17.6KB brain page, `finish=stop`, 0 reasoning tokens. When a script's API call returns empty content, check whether the model is reasoning-class before blaming the prompt; and always bound retries with a `_retried` flag (never infinite recursion).
