...
- **Verse text missing from delivered entries (fleet-wide 2026-09-08..12)** → the accept gate only required a Chapter:Verse citation somewhere in the header, so the model could fill the verse slot with the reference itself (`*"Titus 3:8" (Titus 3:8)*`), leave the template unfilled, or emit mangled citations (`1:15:15`, `[Phm 6:17:17]`) — all accepted and delivered. Fixed 2026-09-12: `_header_is_acceptable()` fails closed unless the header carries REAL quoted verse prose (>=10 chars, Unicode words, no digits-colon, no brackets) AND a sane `Book Chapter:Verse` citation (one colon, no brackets, ranges allowed). Rejection feedback names the precise defect; `_strip_rejection_feedback()` removes echoed retry-prompt text that leaked into delivered entries. Tests: `tests/test_bible_verse_guard.py` (20, incl. historical fleet regressions).
