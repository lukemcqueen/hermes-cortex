...
- `orch-team-messages.sh` lines 75-82 (sed/grep extraction)
...
**Suggested fix:** Either (a) centralize frontmatter parsing in a shared Python module imported by both server.py and inbox-sensor.py, and have orch-team-messages.sh use `python3 -c` for parsing, or (b) standardize on JSON message files instead of YAML frontmatter in markdown.
