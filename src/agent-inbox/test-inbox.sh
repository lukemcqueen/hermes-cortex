...
CERT_INFO=$(echo | openssl s_client -connect 127.0.0.1:13004 \
  -servername your-domain.com 2>/dev/null | openssl x509 -noout -subject -dates 2>/dev/null)
if echo "$CERT_INFO" | grep -q "your-domain.com"; then
  pass "SSL cert is for your-domain.com"
else
