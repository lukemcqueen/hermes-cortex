...
if _CONF_FILE.exists():
    ...
    for line in _CONF_FILE.read_text().splitlines():
        if "=" in line:
            ...
            conf[k] = v
    user = conf.get("AGENT_INBOX_USER", "")
    pw = conf.get("AGENT_INBOX_PASS", "")
    if user and pw:
        import base64
        token = base64.b64encode(f"{user}:{pw}".encode()).decode()
        _INBOX_AUTH_HEADER = f"Basic {token}"
