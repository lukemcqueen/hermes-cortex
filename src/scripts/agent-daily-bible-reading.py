...
def find_all_entries() -> list[tuple[int, str, str]] | None:
    """Parse SOUL.md and return list of (line_number, book_name, full_entry_text) for each entry.
    ...
    while i < end:
        ...
        if m:
            name = m.group(1).strip()
            if name in BOOK_INDEX:
                # Collect all lines until next ### or end of section
                entry_lines = []
                while i < end:
...
def archive_old_entries(entries: list[tuple[int, str, str]]) -> bool:
    ...
    # Entries to archive (oldest ones, excluding the last MAX_INLINE_ENTRIES)
    ...
    # Find next archive file number
    existing = list(BIBLE_ARCHIVE_DIR.glob("archive-*.md"))
    next_num = len(existing) + 1

    book_range = f"{to_archive[0][1]} \u2192 {to_archive[-1][1]}"
    ...
    for _, _, entry_text in to_archive:
    ...
    # Now trim SOUL.md: rebuild the Scripture Insights section with only kept entries
    text = SOUL_MD.read_text(encoding="utf-8")
    lines = text.split("\n")
    ...
    # Build new section: header, comment, pointer to archive, then kept entries
    new_section = [
        "## Scripture Insights",
        "",
...
def main() -> int:
    ...
    # Archive old entries if SOUL.md has grown too many inline readings
