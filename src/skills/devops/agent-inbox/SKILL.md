...
`~/hermes-cortex/src/agent-inbox/server.py` — FastAPI app running on `127.0.0.1:8903` behind nginx basic auth.

Storage: Markdown files with YAML frontmatter in the private repo at `~/hermes-cortex-private/messages/inbox/` and `~/hermes-cortex-private/messages/processed/`.
...
Generated by: `generate-inbox-wrappers.py` in `~/hermes-cortex/src/scripts/`
...
4. Run: `python3 ~/hermes-cortex/src/scripts/generate-inbox-wrappers.py --apply-crons`
