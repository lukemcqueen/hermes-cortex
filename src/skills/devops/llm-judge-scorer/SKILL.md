...
A `no_agent` cron script that evaluates Hermes conversation traces in Langfuse using `qwen2.5-coder:1.5b` as the judge model. Scores are posted back to Langfuse under `helpfulness`, `clarity`, `depth`, and `overall`.
...
llm-judge-scorer.py (no_agent cron)
    ...
    ├── Sends input/output to Ollama (qwen2.5-coder:1.5b)
...
| Judge model | `ollama list \| grep qwen2.5-coder:1.5b` | `ollama pull qwen2.5-coder:1.5b` |
...
| `Model not found` | `ollama pull qwen2.5-coder:1.5b` |
| Zero traces scored | Normal if all traces already scored. Check Langfuse has traces. |

## Deployment

Script lives at `src/scripts/llm-judge-scorer.py` in the cortex repo.
